...
- (id)initWithOriginY:(CGFloat)y Width:(CGFloat)width Titles:(NSArray *)titles delegate:(id)delegate selectIndex:(NSInteger)select;
{
    NSLog(@"执行了一次");
...
//        
        ...
        //  bottom lineView
        //
        _bottomLineView = [[UIView alloc] initWithFrame:CGRectMake(5.0f+80*select, HYSegmentedControl_Height-1, width4btn-10.0f, 1.0f)];
...
- (void)segmentedControlChange:(UIButton *)btn
{
    ...
    if ((btn.tag - Define_Tag_add) >= 2 && [_array4Btn count] > 4 && [_array4Btn count] > (btn.tag - Define_Tag_add + 2)) {
        ...
        if ((btn.tag+1-Define_Tag_add==_array4Btn.count)) {
             pt.x = (_array4Btn.count - 3) * Min_Width_4_Button;
        }else
        {
